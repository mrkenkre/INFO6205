...
public class MergeSort<X extends Comparable<X>> extends SortWithHelper<X> {
	...
	private void sort(X[] a, X[] aux, int from, int to) {
		...
		/*
		 * final int items = to - from; int mid = from + items / 2; if (insurance ==
		 * true) { if (helper.less(a[mid - 1], a[mid])) { return; } } if (noCopy) {
		 * sort(aux, a, from, mid); sort(aux, a, mid, to); } else { sort(a, aux, from,
		 * mid); sort(a, aux, mid, to); System.arraycopy(a, from, aux, from, items); }
