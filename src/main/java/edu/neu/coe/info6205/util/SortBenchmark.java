...
public class SortBenchmark {
	...
	private void sortIntegersByShellSort(final int n) {
		...
		final double t1 = new Benchmark_Timer<int[]>("intArraysorter", (xs) -> Arrays.copyOf(xs, xs.length),
				Arrays::sort, null).runFromSupplier(intsSupplier, 100);
		...
		final double t2 = new Benchmark_Timer<Integer[]>("integerArraysorter", (xs) -> Arrays.copyOf(xs, xs.length),
				Arrays::sort, null).runFromSupplier(integersSupplier, 100);
		for (TimeLogger timeLogger : timeLoggersLinearithmic)
			timeLogger.log(t2, n);
	}
...
//        doLeipzigBenchmark("zho-simp-tw_web_2014_10K-sentences.txt", 5000, 1000);
	 ...
	 * comparisons. That's because, in addition to each comparison, there will be
	 * approximately two copy operations. Thus, in the case where comparisons are
	 * based on primitives, the normalized time per run should approximate the time
	 * for one array access.
	 */
	 ...
	 * elements. For insertion sort, each (low-level) swap fixes one inversion, so
	 * on average there are this number of swaps. The minimum number of comparisons
	 * is slightly higher.
	 *
	 * @param n the number of elements
...
//    }
	 ...
	 * the number of comparisons. That's because, for each inversion, there will
	 * typically be one swap (four array accesses) and (at least) one comparison
	 * (two array accesses). Thus, in the case where comparisons are based on
	 * primitives, the normalized time per run should approximate the time for one
	 * array access.
	 */
